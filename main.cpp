...
static void print_tcp(FILE* dst, const struct ip* ip, const struct tcphdr* tcp, qd_real timestamp, int payload_size){
	...
	fprintf(dst, "%s %d %d \n",inet_ntoa(ip->ip_dst),(u_int16_t)ntohs(tcp->dest),payload_size);
         int temp;
...
static void print_udp(FILE* dst, const struct ip* ip, const struct udphdr* udp, qd_real timestamp, int payload_size){
	...
	fprintf(dst, "%s %d %d \n",inet_ntoa(ip->ip_dst),(u_int16_t)ntohs(udp->dest),payload_size);
       // int temp;
...
static void print_icmp(FILE* dst, const struct ip* ip, const struct icmphdr* icmp, int payload_size){
	
        fprintf(dst, "%s -1 ",inet_ntoa(ip->ip_src));
	fprintf(dst, "%s -1 %d\n",inet_ntoa(ip->ip_dst), payload_size);
...
static void print_ipv4(FILE* dst, const struct ip* ip, qd_real timestamp, int payload_size){
	...
	case IPPROTO_ICMP:
		fprintf(dst, "%d ", ip->ip_p);
		print_icmp(dst, ip, (const struct icmphdr*)payload,  payload_size);
		break;

	default:
		fprintf(dst, "%d %s -1 %s -1 %d\n", ip->ip_p,inet_ntoa(ip->ip_src),inet_ntoa(ip->ip_dst),  payload_size);
		break;
	}
}
